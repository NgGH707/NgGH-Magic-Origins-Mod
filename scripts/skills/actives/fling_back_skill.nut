this.fling_back_skill <- this.inherit("scripts/skills/skill", {
	...
	function onPerformAttack( _tag )
	{
		...
		local mult = this.isUpgraded() ? 2.0 : 1.0;
	...
	function onKnockedDown( _entity, _tag )
	{
		...
		if (_tag.Skill.isUpgraded() && _entity.isAlive() && !_entity.isDying() && !_entity.getCurrentProperties().IsImmuneToStun)
		{
	...
	function onAfterUpdate( _properties )
	{
		...
		this.m.ActionPointCost = _properties.IsSpecializedInThrowing ? 3 : 4;
	}
