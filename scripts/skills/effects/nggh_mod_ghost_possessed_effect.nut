this.nggh_mod_ghost_possessed_effect <- ::inherit("scripts/skills/skill", {
	m = {
		TurnsLeft = 1,
		OriginalFaction = 0,
		OriginalAgent = null,
	...
	function onPossess()
	{
		...
		AI.addBehavior(::new("scripts/ai/tactical/behaviors/ai_ghost_possess"));
		AI.addBehavior(::new("scripts/ai/tactical/behaviors/ai_attack_terror"));
		...
		local scream = this.m.Possessor.getType() == ::Const.EntityType.LegendBanshee ? ::new("scripts/skills/actives/legend_banshee_scream") : ::new("scripts/skills/actives/horrific_scream");
	...
	function onTurnStart()
	{
		...
		if (myTile.Properties.Effect != null && myTile.Properties.Effect.Type == "legend_holyflame")
		{
			...
			this.removeSelf();
			::Tactical.EventLog.log(::Const.UI.getColorizedEntityName(this.m.Possessor) + " has been banished out of " + ::Const.UI.getColorizedEntityName(actor) + "\'s body");
			return;
		}
	...
	function onOtherActorDeath( _killer, _victim, _skill, _deathTile, _corpseTile, _fatalityType )
	{
		...
		{
			this.m.TurnsLeft = ::Tactical.Entities.getHostilesNum() <= 1 ? 99 : this.m.TurnsLeft;
		}
	}
