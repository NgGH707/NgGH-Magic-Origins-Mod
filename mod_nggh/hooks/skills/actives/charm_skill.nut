::Nggh_MagicConcept.HooksMod.hook("scripts/skills/actives/charm_skill", function ( q )
{
	/*
	q.onVerifyTarget <- function( _originTile, _targetTile )
	{
		return skill.onVerifyTarget(_originTile, _targetTile) && isViableTarget(getContainer().getActor(), _targetTile.getEntity());
	}
	*/
