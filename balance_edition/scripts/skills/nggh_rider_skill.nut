this.nggh_rider_skill <- this.inherit("scripts/skills/skill", {
	m = {
		QuirksName = [],
		Manager = null,
		IsInBattle = false,
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "All active skills cost [color=" + this.Const.UI.Color.NegativeValue + "]1[/color] more AP"
		});
	...
	function onUpdate( _properties )
	{
		if (this.isMounted())
		{
			...
			actor.m.ActionPointCosts = this.Const.GoblinRider.RiderMovementAPCost;
			actor.m.FatigueCosts = this.Const.GoblinRider.RiderMovementFatigueCost;
		}
	}
