this.mod_summon_mirror_image_skill <- this.inherit("scripts/skills/skill", {
	m = {
		IsSpent = false,
	},
	function create()
	...
	function onAdded()
	{
		this.getContainer().getActor().getAIAgent().addBehavior(this.new("scripts/ai/tactical/behaviors/ai_mirror_image_player"));
	}

	function onRemoved()
	{
		this.getContainer().getActor().getAIAgent().removeBehavior(this.Const.AI.Behavior.ID.MirrorImage);
	}
	...
	function onUse( _user, _targetTile )
	{
		...
		local b = _user.getAIAgent().getBehavior(this.Const.AI.Behavior.ID.MirrorImage);
