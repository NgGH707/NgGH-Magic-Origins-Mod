this.alp_teleport_skill <- this.inherit("scripts/skills/skill", {
	...
	function setUseForFree( _f )
	{
		this.m.IsUseForFree = _f;
	}
	...
	function getTooltip()
	{
		...
		if (!this.m.IsAtNight)
		{
			ret.push({
				...
				text = "Builds up [color=" + this.Const.UI.Color.NegativeValue + "]33%[/color] more fatigue due to daytime"
			});
		}
