this.mod_mind_break_skill <- this.inherit("scripts/skills/skill", {
	m = {},
	function create()
	{
	...
	function onUse( _user, _targetTile )
	{
		...
		this.Tactical.spawnSpriteEffect("effect_skull_03", this.createColor("#ffffff"), _targetTile, 0, 40, 1.0, 0.25, 0, 400, 300);
	...
	function onDelayedEffect( _tag )
	{
		...
		if (ret && _targetEntity.isAlive() && !_targetEntity.isDying())
		{
			...
			if (mind_break != null)
			{
				_targetEntity.checkMorale(-1, 30 - mind_break.m.Count, this.Const.MoraleCheckType.MentalAttack);
			}
