this.mod_kraken_spawn_tentacle_skill <- this.inherit("scripts/skills/skill", {
	m = {},
	function create()
	{
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "Current tentacle(s): [color=" + this.Const.UI.Color.PositiveValue + "]" + this.getContainer().getActor().getTentacles().len() + "[/color]"
		});
		return ret;
	}
	...
	{
		this.Time.scheduleEvent(this.TimeUnit.Virtual, 500, _user.spawnTentacle.bindenv(_user.get()), _targetTile);
		return true;
	}
