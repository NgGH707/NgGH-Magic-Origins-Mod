this.mod_ghost_possess_player <- this.inherit("scripts/skills/skill", {
	m = {
		IsSpent = false
	},
	function create()
	...
	function onDelayedEffect( _tag )
	{
		...
		local checkLastEnemy = this.Tactical.Entities.getHostilesNum() == 1;
		local possessed = this.new("scripts/skills/effects/mod_ghost_possessed_player_effect");
		...
		{
			this.Tactical.getRetreatRoster().add(_user);
		}
		...
		this.Time.scheduleEvent(this.TimeUnit.Virtual, time, function ( _e )
		{
			if (checkLastEnemy)
			{
				target.kill(_user, _e, this.Const.FatalityType.Suicide, true);
			}
			else
			{
