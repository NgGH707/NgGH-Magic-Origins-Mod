this.mod_summon_flying_skulls_skill <- this.inherit("scripts/skills/skill", {
	m = {
		Cooldown = 0
	},
	function create()
	...
	function onAdded()
	{
		this.getContainer().getActor().getAIAgent().addBehavior(this.new("scripts/ai/tactical/behaviors/ai_flying_skulls_player"));
	}

	function onRemoved()
	{
		this.getContainer().getActor().getAIAgent().removeBehavior(this.Const.AI.Behavior.ID.FlyingSkulls);
	}
		...
		if (this.Tactical.isActive())
		{
			...
			if (count > 0)
			{
				ret.push({
					...
					text = "There is [color=" + this.Const.UI.Color.PositiveValue + "]" + count + "[/color] 'Screaming Skull(s)' on the map"
				});
			}
			else
			{
			...
			if (this.m.Cooldown == 0 && count > 6)
			{
				ret.push({
					...
					text = "[color=" + this.Const.UI.Color.NegativeValue + "]Can only spawn more when there is less than 7 'Screaming Skulls'[/color]"
				});
			}
		}
	...
	function onUse( _user, _targetTile )
	{
		...
		local b = _user.getAIAgent().getBehavior(this.Const.AI.Behavior.ID.FlyingSkulls);
