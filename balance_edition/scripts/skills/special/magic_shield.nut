this.magic_shield <- this.inherit("scripts/skills/skill", {
	m = {
		HitPoints = 50,
		HitPointsMax = 100,
		IsActivated = true,
	...
	function onBeforeDamageReceived( _attacker, _skill, _hitInfo, _properties )
	{
		...
		{
			damage = this.Math.max(1, this.Math.floor(_hitInfo.DamageArmor * 0.5));
			mult = 1.0;
		}
		...
		{
			_hitInfo.DamageRegular = this.Math.max(1, overflow);
