...
this.charm_captive_spell <- this.inherit("scripts/skills/mc_magic_skill", {
	...
	function getTooltip()
	{
		...
		if (this.Tactical.isActive() && ("StrategicProperties" in this.Tactical.State.m) && this.Tactical.State.m.StrategicProperties != null && this.Tactical.State.m.StrategicProperties.IsArenaMode)
		{
			ret.push({
				id = 99,
				type = "text",
				icon = "ui/tooltips/warning.png",
				text = "[color=" + this.Const.UI.Color.NegativeValue + "]You are not allowed use this skill in arena[/color]"
			});
			return ret;
		}
	...
	function onCombatStarted()
	{
		...
		if (("StrategicProperties" in this.Tactical.State.m) && this.Tactical.State.m.StrategicProperties != null && this.Tactical.State.m.StrategicProperties.IsArenaMode)
		{
			this.m.Count = 0;
			return;
		}
