this.mod_ghost_possessed_player_effect <- this.inherit("scripts/skills/skill", {
	m = {
		TurnsLeft = 1,
		OriginalFaction = 0,
		OriginalAgent = null,
		OriginalSocket = null,
		PossessorFaction = this.Const.Faction.PlayerAnimals,
		Possessor = null,
		IsActivated = false,
		IsBattleEnd = false,
		IsEnhanced = false,
	...
	function onSpawnGhost( _info )
	{
		...
		this.Tactical.getRetreatRoster().remove(_skill.m.Possessor);
	...
	function onPossess()
	{
		...
		local depossess = this.new("scripts/skills/actives/mod_ghost_depossess");
		...
		local scream = this.m.Possessor.getType() == this.Const.EntityType.LegendBanshee ? this.new("scripts/skills/actives/mod_banshee_scream") : this.new("scripts/skills/actives/mod_horrific_scream");
	...
	function onTargetKilled( _targetEntity, _skill )
	{
		...
		local XPkiller = this.Math.floor(_targetEntity.getXPValue() * this.Const.XP.XPForKillerPct);
		local XPgroup = _targetEntity.getXPValue() * (1.0 - this.Const.XP.XPForKillerPct);
	...
	{
		if (this.m.IsActivated && _damageHitpoints >= this.Const.Combat.InjuryMinDamage && this.Math.rand(1, 100) <= 10)
		{
			this.removeSelf();
