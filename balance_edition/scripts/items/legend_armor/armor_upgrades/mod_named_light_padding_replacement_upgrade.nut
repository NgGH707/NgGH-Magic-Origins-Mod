this.mod_named_light_padding_replacement_upgrade <- this.inherit("scripts/items/legend_armor/legend_named_armor_upgrade", {
	m = {
	...
	{
		this.m.SpecialValue = this.Math.min(30, this.Math.floor(this.m.SpecialValue * this.Math.rand(110, 130) * 0.01));
	...
	function onEquip()
	{
		this.m.StaminaModifier = -this.Math.floor(this.m.Armor.getStaminaModifier() * this.m.SpecialValue * 0.01);
