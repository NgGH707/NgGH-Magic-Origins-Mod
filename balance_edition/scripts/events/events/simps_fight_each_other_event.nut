this.simps_fight_each_other_event <- this.inherit("scripts/events/event", {
	m = {
		Hexe = null,
		Simp1 = null,
		Simp2 = null,
	...
	function create()
	{
		...
		this.m.Screens.push({
			...
			function start( _event )
			{
				...
				this.List.push({
					id = 10,
					icon = this.Const.MoodStateIcon[_event.m.Simp1.getMoodState()],
				...
				this.List.push({
					id = 10,
					icon = this.Const.MoodStateIcon[_event.m.Simp2.getMoodState()],
	...
	function onPrepare()
	{
		...
		local candidates = [];
		this.m.Hexe = hexe[this.Math.rand(0, hexe.len() - 1)];
	...
	function getSimpLib()
	{
		return [
			...
			{
				ID = "effects.fake_1_charmed",
				Script = "scripts/skills/effects/fake_charmed_1_effect"
			},
			{
				ID = "effects.fake_2_charmed",
				Script = "scripts/skills/effects/fake_charmed_2_effect"
			},
			{
				ID = "effects.fake_3_charmed",
				Script = "scripts/skills/effects/fake_charmed_3_effect"
			},
		];
	}

	function increaseSimpLevel( _simp )
	{
		local simpLib = this.getSimpLib();
		local ret;
		local index;

		foreach (i, _level in simpLib ) 
		{
		    if (_simp.getSkills().hasSkill(_level.ID))
		    {
		    	_simp.getSkills().removeByID(_level.ID);
		    	index = i + 1;
		    	break;
		    }
		}
		...
		if (index != null)
		{
			...
			ret = this.new(simpLib[index].Script);
			_simp.getSkills().add(ret);
			return ret;
		}
	...
	function decreaseSimpLevel( _simp )
	{
		local simpLib = this.getSimpLib();
		local ret;
		local index;
