this.suicide_event <- this.inherit("scripts/events/event", {
	m = {
		Suicider = null,
		Other = null
	},
	function setSuicider( _d )
	{
		...
		if (brothers.len() > 1)
		{
			...
			while (this.m.Other == null || this.m.Other.getID() == this.m.Suicider.getID());
		}
		else
		{
			...
			Options = [
				{
					...
					{
						_event.m.Suicider.getItems().transferToStash(this.World.Assets.getStash());
						this.World.Statistics.addFallen(_event.m.Suicider, "Suicided");
						this.World.getPlayerRoster().remove(_event.m.Suicider);
