this.hexen_intro_event <- this.inherit("scripts/events/event", {
	m = {
		Hexe = null
	},
	function create()
	{
		this.m.ID = "event.hexen_intro_event";
	...
	function onPrepare()
	{
		...
		{
			if (b.getBackground().getID() == "background.hexen" || b.getFlags().has("Hexe"))
			{
				this.m.Hexe = b;
				break;
			}
	...
	function onPrepareVariables( _vars )
	{
		...
		foreach (b in brothers)
		{
			if (b.getBackground().getID() == "background.hexen_commander" || b.getFlags().has("Hexe"))
			{
				hexe= b;
				break;
			}
